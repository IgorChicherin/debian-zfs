...
set_dataset_properties() {
    ...
    # Required for ZFSBootMenu boot environment discovery
    # Avoid changing mount behavior for currently mounted dataset (can fail with '/mnt busy')
    if [ "$mounted_state" = "yes" ] && [ -n "$current_mountpoint" ] && [ "$current_mountpoint" != "/" ]; then
        log_warn "Dataset currently mounted at $current_mountpoint"
        log_warn "Skipping mountpoint/canmount change to avoid busy unmount error"
        log_info "Apply later after unmount if needed:"
        log_info "  zfs set mountpoint=/ $dataset"
        log_info "  zfs set canmount=noauto $dataset"
    else
        log_info "Setting mountpoint=/ and canmount=noauto..."
        zfs set mountpoint=/ "$dataset"
        zfs set canmount=noauto "$dataset"
    fi

    log_info "Setting pool bootfs..."
    ...
    # Check encryption
    local encryption
    encryption=$(zfs get -H -o value encryption "$dataset" 2>/dev/null || echo "off")
...
download_zfsbootmenu() {
    ...
    # Download EFI binary with proper error checking
    ...
    if ! curl -fSL --connect-timeout 10 -m 300 -o "$ZBM_DIR/VMLINUZ.EFI" \
        ...
        log_info "You can manually download from: https://github.com/zbm-dev/zfsbootmenu/releases"
        exit 1
    fi
    ...
    # Create UEFI fallback path
    log_info "Creating fallback EFI path..."
    mkdir -p /boot/efi/EFI/BOOT
    cp "$ZBM_DIR/VMLINUZ.EFI" /boot/efi/EFI/BOOT/BOOTX64.EFI
...
create_efi_entries() {
    ...
    # Remove old entries (if updating) with more robust parsing
    if [ "$UPDATE_MODE" = true ]; then
        log_info "Removing old ZFSBootMenu entries..."
        # Better parsing: efibootmgr output format is "Boot0000* Name"
        efibootmgr | grep "ZFSBootMenu" | sed 's/^Boot//' | sed 's/\*.*//' | sed 's/ .*//' | while read -r num; do
            if [ -n "$num" ] && [[ "$num" =~ ^[0-9]+$ ]]; then
                log_info "  Removing Boot$num..."
    ...
    if ! efibootmgr -c -d "$BOOT_DISK" -p "$BOOT_PART" \
        -L "ZFSBootMenu (Backup)" \
        -l '\EFI\ZBM\VMLINUZ-BACKUP.EFI' 2>/dev/null; then
        log_warn "Failed to create backup EFI entry (may need manual configuration)"
    fi
    ...
    if ! efibootmgr -c -d "$BOOT_DISK" -p "$BOOT_PART" \
        -L "ZFSBootMenu" \
        -l '\EFI\ZBM\VMLINUZ.EFI' 2>/dev/null; then
        log_warn "Failed to create main EFI entry (may need manual configuration)"
    fi
...
main() {
    ...
    log_info "  /boot/efi/EFI/BOOT/BOOTX64.EFI"
    log_info ""
